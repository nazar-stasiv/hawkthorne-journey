...
function test_osx_get_application_path()
  ...
  lunatest.assert_equal("/Users/joe/projects/hawkthorne-journey/Journey to the Center of Hawkthorne.app", path)
end

function test_osx_short_path()
  local path = osx.getApplicationPath("//")
  lunatest.assert_equal(path, "")
end
...
function test_updater_no_thread_started()
  local u = sparkle.newUpdater("0.0.0", "")
  u:start()
  lunatest.assert_nil(u.thread)
end

function test_updater_progress_not_started()
  local u = sparkle.newUpdater("0.0.0", "http://example.com")
  local msg, percent = u:progress()
  lunatest.assert_equal("Waiting to start", msg)
  lunatest.assert_equal(0, percent)
end
...
function test_sparkle_same_versions()
  lunatest.assert_false(sparkle.isNewer("0.0.1", "0.0.1"))
end

function test_sparkle_lower_versions()
  lunatest.assert_false(sparkle.isNewer("0.0.1", "0.0.0"))
end

function test_sparkle_higher_bug_versions()
  lunatest.assert_true(sparkle.isNewer("0.0.1", "0.0.2"))
end

function test_sparkle_lower_minor_version()
  lunatest.assert_false(sparkle.isNewer("0.1.0", "0.0.2"))
end

function test_sparkle_unsupported_versions()
  lunatest.assert_false(sparkle.isNewer("0.0.-1", "0.0.2.0"))
end

function test_sparkle_higher_minor_version()
  lunatest.assert_true(sparkle.isNewer("0.1.0", "0.2.0"))
end

function test_sparkle_lower_major_version()
  lunatest.assert_false(sparkle.isNewer("1.0.0", "0.2.0"))
end

function test_sparkle_higher_major_version()
  lunatest.assert_true(sparkle.isNewer("1.9.9", "2.0.0"))
end

function test_sparkle_remove_path_no_exist()
  lunatest.assert_true(windows.removeRecursive("nonexistant"))
end
...
function test_sparkle_remove_directory()
  ...
  lunatest.assert_true(windows.removeRecursive("test_folder"))
  lunatest.assert_false(love.filesystem.getInfo("test_folder/foo.txt"))
end
...
function test_sparkle_windows_basename()
  ...
  lunatest.assert_equal("OpenAL32.dll", basename)
end
